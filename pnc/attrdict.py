...
class AttrDict(dict):
    ...
    def __setattr__(self, name, value):
        ...
        if self._hasattr(name, local=False):
    ...
    def __delattr__(self, name):
        ...
        # __slots__.
        if self._hasattr(self, local=True):
            # See __setattr__.
    ...
    def _hasattr(self, name, local=False):
        """Check if attribute 'name' is specifically defined on this object or
        its classes.
        Used to determine whether an attribute should be set to __dict__ or
        added to the object's conventional dictionary.
        If 'local', only check __dict__."""
        if not local:
            return name in dir(self) or self._is_reserved(name)
        else:
            return name in self.__dict__
        # Just to be tidy/in case more options are added.
        return False
